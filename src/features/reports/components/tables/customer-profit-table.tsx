...
import { CustomerProfitRow } from '../../domain/reports-types';
...
interface CustomerProfitTableProps {
  data: CustomerProfitRow[];
  isLoading: boolean;
}

type SortField = keyof CustomerProfitRow;

export function CustomerProfitTable({ data, isLoading }: CustomerProfitTableProps) {
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Breakdown of profit by customer</CardDescription>
          </div>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2">
        ...
        <ReportExportToolbar data={sortedData} columns={columns} filename="customer-profitability" />
      </CardHeader>
      <CardContent>
              ...
              {sortedData.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center h-24 text-muted-foreground">
                    No customer profitability data found for this period.
                  </TableCell>
                </TableRow>
              ) : (
                sortedData.map((row) => (
