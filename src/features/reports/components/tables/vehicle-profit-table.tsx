...
import { VehicleProfitRow } from '../../domain/reports-types';
...
interface VehicleProfitTableProps {
  data: VehicleProfitRow[];
  isLoading: boolean;
}

type SortField = keyof VehicleProfitRow;

export function VehicleProfitTable({ data, isLoading }: VehicleProfitTableProps) {
  ...
  const columns = [
    ...
    { header: 'Profit / Trip', key: 'profit_per_trip' },
  ];
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Breakdown of profit by vehicle</CardDescription>
          </div>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2">
        ...
        <ReportExportToolbar data={sortedData} columns={columns} filename="vehicle-profitability" />
      </CardHeader>
      <CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">
                  <Button variant="ghost" size="sm" onClick={() => handleSort('profit_per_trip')} className="h-8 flex items-center justify-end w-full gap-1">
                    Profit / Trip <ArrowUpDown className="h-3 w-3" />
                  </Button>
                </TableHead>
              </TableRow>
            </TableHeader>
              ...
              {sortedData.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center h-24 text-muted-foreground">
                    No vehicle profitability data found for this period.
                  </TableCell>
                </TableRow>
              ) : (
                sortedData.map((row) => (
                  <TableRow key={row.vehicle_id}>
                    ...
                    <TableCell className="text-right font-medium">{formatCurrency(row.profit_per_trip)}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
