...
export const companySchema = z.object({
  ...
  contact_person: optionalString(
    z.string().trim().min(2, 'Contact person name must be at least 2 characters').max(100, 'Name cannot exceed 100 characters')
  ),
  ...
  address: optionalString(
    z.string().trim().min(5, 'Address must be at least 5 characters').max(500, 'Address cannot exceed 500 characters')
  ),
  
  city: optionalString(
    z.string().trim().min(2, 'City must be at least 2 characters').max(100, 'City cannot exceed 100 characters')
  ),
  
  state: optionalString(
    z.string().trim().min(2, 'State must be at least 2 characters').max(100, 'State cannot exceed 100 characters')
  ),
